...
from consts import BASE_KEYPOINT, RIGHT_BODY_KEYPOINTS, LEFT_BODY_KEYPOINTS, LEFT_HAND_KEYPOINTS, RIGHT_HAND_KEYPOINTS, LINE_WIDTH_CONST


def plot_body_right_keypoints(keypoints, alpha=None, line_width=LINE_WIDTH_CONST, bias=0):
    _keypoints = np.array(BASE_KEYPOINT + RIGHT_BODY_KEYPOINTS)
    plt.plot(keypoints[0][_keypoints]+bias, keypoints[1][_keypoints], linewidth=line_width, alpha=alpha,
             color='red')


def plot_body_left_keypoints(keypoints, alpha=None, line_width=LINE_WIDTH_CONST, bias=0):
    _keypoints = np.array(BASE_KEYPOINT + LEFT_BODY_KEYPOINTS)
    plt.plot(keypoints[0][_keypoints]+bias, keypoints[1][_keypoints], linewidth=line_width, alpha=alpha,
             color='blue')


def plot_left_hand_keypoints(keypoints, alpha=None, line_width=LINE_WIDTH_CONST, bias=0):
    for i in range(5):
...
def plot_right_hand_keypoints(keypoints, alpha=None, line_width=LINE_WIDTH_CONST, bias=0):
    for i in range(5):
        _keypoints = np.array(RIGHT_HAND_KEYPOINTS(i))
        plt.plot(keypoints[0][_keypoints]+bias, keypoints[1][_keypoints], linewidth=line_width, alpha=alpha,
                 color='black')

def make_image(keypoints1,keypoints2,output,img_size=(360,1280)):
    plt.figure()
    plt.axis('off')
    plt.title('test')
    img = Image.new(mode='RGB', size=img_size, color='white')
    plt.imshow(img)
    plot_body_right_keypoints(keypoints1)
    plot_body_left_keypoints(keypoints1)
    # plot_left_hand_keypoints(keypoints1)
    # plot_right_hand_keypoints(keypoints1)

    plot_body_right_keypoints(keypoints2,bias=640)
    plot_body_left_keypoints(keypoints2,bias=640)
    # plot_left_hand_keypoints(keypoints2,bias=640)
    # plot_right_hand_keypoints(keypoints2,bias=640)

    plt.savefig(output)
    plt.close()

def make_video(pred_keypoints,gt_keypoints):
    if not os.path.exists('make_video/video'):
        os.makedirs('make_video/video')
    if not os.path.exists('make_video/tmp_img'):
        os.makedirs('make_video/tmp_img')

    output_pattern = os.path.join('make_video/tmp_img','%04d.jpg')

    for i in range(pred_keypoints.shape[1]):
        make_image(pred_keypoints[:,i,:],gt_keypoints[:,i,:],output_pattern % (i))
    
    subprocess.call('ffmpeg -loglevel panic -r 30000/2002 -f image2 -i "%s" -r 30000/1001 "%s" -y' % (
        os.path.join('make_video/tmp_img','%04d.jpg'), 'make_video/video/v.mp4'), shell=True)
